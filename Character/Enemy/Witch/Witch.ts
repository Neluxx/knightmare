namespace Game {
  ...
  export class Witch extends Enemy {
    ...
    constructor() {
      super();
      this.health = data.witch.health;
      this.strength = data.witch.strength;
      this.attackspeed = data.witch.attackspeed; //in ms
      this.speedMax = new ƒ.Vector2(data.witch.speedMax, 5); //units per second

      this.addComponent(new ƒ.ComponentTransform());
      this.show(ACTION.WITCH_IDLE);
      ƒ.Loop.addEventListener(ƒ.EVENT.LOOP_FRAME, this.update);
    }

    public act(_action: ACTION, _direction?: DIRECTION): void {
      switch (_action) {
        case ACTION.WITCH_ATTACK:
          this.direct = (_direction == DIRECTION.RIGHT ? 1 : -1);
          this.cmpTransform.local.rotation = ƒ.Vector3.Y(90 - 90 * this.direct);
          this.speed.x = 0;
          break;
        case ACTION.WITCH_IDLE:
          this.speed.x = 0;
          break;
        case ACTION.WITCH_DIE:
          this.speed.x = 0;
          setTimeout(() => {
    ...
    private playerDetection(): void {
      if (Math.abs(player.cmpTransform.local.translation.x - this.cmpTransform.local.translation.x) < 3 && Math.abs(player.cmpTransform.local.translation.x - this.cmpTransform.local.translation.x) > 1.1 && !gameOver) {
        if (player.cmpTransform.local.translation.x > this.cmpTransform.local.translation.x) {
          this.act(ACTION.WITCH_WALK, DIRECTION.RIGHT);
        }
        else {
          this.act(ACTION.WITCH_WALK, DIRECTION.LEFT);
        }
      }
      else if (Math.abs(player.cmpTransform.local.translation.x - this.cmpTransform.local.translation.x) < 3 && Math.abs(player.cmpTransform.local.translation.x - this.cmpTransform.local.translation.x) < 1.1 && !gameOver) {
        if (player.cmpTransform.local.translation.x > this.cmpTransform.local.translation.x) {
          this.act(ACTION.WITCH_ATTACK, DIRECTION.RIGHT);
        }
        else {
          this.act(ACTION.WITCH_ATTACK, DIRECTION.LEFT);
        }
      }
      else {
        this.act(ACTION.WITCH_IDLE);
      }
    }

    private update = (_event: ƒ.Eventƒ): void => {
      if (this.isDying) {
        this.show(ACTION.WITCH_DEAD);
        return;
      }
    ...
    public getRectEnemy(): ƒ.Rectangle {
      ...
      let mtxResult: ƒ.Matrix4x4 = ƒ.Matrix4x4.MULTIPLICATION(this.mtxWorld, Witch.pivot);
