namespace Game {
  ...
  export class Wolf extends Enemy {
    ...
    constructor() {
      super();
      this.health = data.wolf.health;
      this.strength = data.wolf.strength;
      this.attackspeed = data.wolf.attackspeed; //in ms
      this.speedMax = new ƒ.Vector2(data.wolf.speedMax, 5); //units per second
    ...
    private playerDetection(): void {
      if (Math.abs(player.cmpTransform.local.translation.x - this.cmpTransform.local.translation.x) < 3 && Math.abs(player.cmpTransform.local.translation.x - this.cmpTransform.local.translation.x) > 1 && !gameOver) {
        if (player.cmpTransform.local.translation.x > this.cmpTransform.local.translation.x) {
          this.act(ACTION.WOLF_WALK, DIRECTION.RIGHT);
        }
        else {
          this.act(ACTION.WOLF_WALK, DIRECTION.LEFT);
        }
      }
      else if (Math.abs(player.cmpTransform.local.translation.x - this.cmpTransform.local.translation.x) < 3 && Math.abs(player.cmpTransform.local.translation.x - this.cmpTransform.local.translation.x) < 1 && !gameOver) {
        if (player.cmpTransform.local.translation.x > this.cmpTransform.local.translation.x) {
          this.act(ACTION.WOLF_ATTACK, DIRECTION.RIGHT);
        }
        else {
          this.act(ACTION.WOLF_ATTACK, DIRECTION.LEFT);
        }
      }
      else {
        if (this.idle) {
          this.act(ACTION.WOLF_IDLE);
        }
        else {
          if (this.moveLeft) {
            this.act(ACTION.WOLF_WALK, DIRECTION.LEFT);
    ...
    public getRectEnemy(): ƒ.Rectangle {
      ...
      let mtxResult: ƒ.Matrix4x4 = ƒ.Matrix4x4.MULTIPLICATION(this.mtxWorld, Wolf.pivot);
